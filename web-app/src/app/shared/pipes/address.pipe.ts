...
export class AddressPipe implements PipeTransform {
  ...
  transform(value?: ConfirmationToken['shipping'] | ShippingAddress, ...args: unknown[]): unknown {
    if(value && 'address' in value && value.name) {
      ...
      return `${value.name}, ${line1}${line2 ? line2 : ''}, ${city}, ${country}, ${state}, ${postal_code}`;
    } else if(value && 'line1' in value) {
      const {line1, line12 , city, state, country, postalCode} = value as ShippingAddress;

      return `${value.name}, ${line1}${line12 ? line12 : ''}, ${city}, ${country}, ${state}, ${postalCode}`;
    } else {
